...
# 
# Usage:  check_IBM_health.sh <DS_controller> [<auth>]
# Params:
#         <DS_controller> - DSxx controller DNS name/IP
#         <auth> - flag to use authentication with SMcli (required for new DSxx FW 10.85+ with locked down Monitor account)
#
...
#append SMcli authentication arguments if <auth> flag is specified
if [[ $2 == 'auth' ]]; then
  AUTH_ARGS="-S -R monitor -p gT7BN!aL"
fi
...
## execute SMcli ###
RESULT=$($COMMAND $1 $AUTH_ARGS -c "show storageSubsystem healthStatus;")
